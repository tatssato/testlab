...
let testlabDNA = path.join('../testlab.dna.gz');

const installAgent: InstallAgentsHapps = [
  [[testlabDNA]],
  [[testlabDNA]],
] 
...
orchestrator.registerScenario('link from private to public', async (s: ScenarioApi, t) => {
  const [conductor] = await s.players([config]);
  const [[alice_lobby_happ], [bobby_lobby_happ]] = await conductor.installAgentsHapps(installAgent);
  const [alice_cell] = alice_lobby_happ.cells;
  const [bobby_cell] = bobby_lobby_happ.cells;
  
  await alice_cell.call('zomeone', 'create_and_link_foofoo', null);
  let foo = await alice_cell.call('zomeone', 'get_foo', null);
  console.log("---------is link working?---------")
  console.log(foo);
});
